...
Для каждого выбранного блюда:
- pitch — живая обоснованная фраза (1–2 предложения), которую официант скажет гостю ДОСЛОВНО: связывает выбор гостя и твоё предложение и объясняет, почему подходит. Пример: «Вы взяли пиццу пепперони — освежит лёгкий овощной салат, он разбавит сытное тесто и сыр.» Или: «К стейку идеально подойдёт бокал красного — подчеркнёт вкус мяса.»
- addon — необязательная вторая пара (например, к десерту — кофе) или null.

Верни СТРОГО JSON: {"suggestions":[{"dishId":"<id из списка>","pitch":"<фраза, что сказать гостю>","addon":"<пара или null>"}]}. Только ВЫБРАННЫЕ блюда, в порядке предпочтения. Бери id ТОЛЬКО из списка кандидатов, не выдумывай.`
